...
SMP자료는 2016년도부터 현재까지 월별 거래현황을 분석하여 나타내었으며, 최근 1주일간 시간대별 SMP의 거래가격등을 분석하였습니다.
...
최근 에너지관련 기사를 형태소분석을 통하여 워드 클라우드 형태로 표현하였습니다. 최근에 가장 많이 ISSUED된 단어를 살펴봄으로서 현재 에너지의 방향을 조금이라도 이해할 수 있도록 하였습니다.  
...
smp_avg_monthly$year <- c("2016","2017","2018","2019","2020","avg")
monthly_year <- 2016
...
#### **2.2 월별 SMP 현황 (2016년~현재)**  
년간 월별 평균 SMP 단가를 분석하였으며, 2016년부터 현재까지 월별 평균 단가도 함께 분석하였습니다.  
...
#### **2.3 SMP 일별현황 (2016년~현재)**  
2016년부터 현재까지 SMP 일별 평균 단가의 Trend를 분석하였습니다.
...
abline()
text(365/2,0,labels = "2016년")
text(365+(365/2),0,labels = "2017년")
text(365+365+(365/2),0,labels = "2018년")
text(365+365+365+(365/2),0,labels = "2019년")
text(365+365+365+365+(365/2),0,labels = "2020년")
...
rec_data.columns = ["sale_date"]
if (file_name[0:2] == '2020'):  #년도가 2020년도로 변경됨에 따라 년도인식이 앞의 20자리와 뒤의 20자리가 동일하여 2020으로 인식할 수 있도록 수정(20200112)
...
rec_monthly$year <- c("2016","2017","2018","2019",2020,"avg")
...
2016년 3월부터 현재까지 [육지]의 월별 평균 REC 단가 및 실적을 분석하였습니다.  
...
2016년 3월부터 현재까지 [육지]의 REC의 차수별  거래단가와 거래량에 대한 Trend를 분석하였습니다.  
...
plot(rec_raw[,3], type = "l", pch = 2, col = "blue", axes = FALSE, xlab = "", 
    ylab = "",xlim=c(0,412),ylim = c(0, 250000))
...
plot(rec_raw[,4], type = "h", pch = 1, col = "red", axes = FALSE, xlab = "", 
    ylab = "",xlim=c(0,412),ylim = c(0, 300000))
#axis(side = 1, at = 2010:2013, labels = year)
#mtext("Year", side = 1, line = 2)
axis(side = 4, col = "red", col.axis = "red")
#mtext("REC 거래량(REC)", side = 5, col = "red", line = 2.5)
...
abline(v=315,col="black",lty=1)

abline()
text(42/2,0,labels = "2016년")
text((42+78/2),0,labels = "2017년")
text((42+78+96/2),0,labels = "2018년")
text((42+78+96+78/2),0,labels = "2019년")
text((42+78+96+85+80),0,labels = "2020년")
...
아래의 공급인증서 발급대상 설비기준은 2020년 7월 1일 개정된 내용을 발췌하였습니다.


```{r,echo=FALSE, out.width="97%", fig.align='center',message=FALSE,warning=FALSE,comment=NA}

library(knitr)
include_graphics("./REC_20200701.jpg")
...
plot(oil_raw1[,2],type="l",xlim=c(0,104), ylim=c(0,100), col="red", ylab="OIL PRICE($/bbl)",xlab="",xaxt="n",yaxt="n")
...
abline()
text(13/2,-1,labels = "2019-1Q")
text(13+13/2,-1,labels = "2019-2Q")
text(13+13+13/2,-1,labels = "2019-3Q")
text(13+13+13+13/2,-1,labels = "2019-4Q")
text(13+13+13+13+13/2,-1,labels = "2020-1Q")
text(13+13+13+13+13+13/2,-1,labels = "2020-2Q")
text(13+13+13+13+13+13+13/2,-1,labels = "2020-3Q")
text(13+13+13+13+13+13+13+13/2,-1,labels = "2020-4Q")
...
# 분석에 불필요한 단어 삭제 
stop_words = ['에너지','분야','통해','위해','라며','한국',"사업",'대한','연합뉴스','가나다','라마','바사','지역','활용','확대','추진','또한','지난','경우','정부','신문','이후','관련','최근','크게','기자']
...
```{r echo=FALSE, fig.width=11, fig.height=5,message=FALSE,warning=FALSE,comment=NA}
library(knitr)
include_graphics("./wordcloud.jpg")
```  
